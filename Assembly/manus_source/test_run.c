...
t_transcription_parameters *transcription_parameters,  t_flags	*has_flag)
{
    int file;
    ...
    close(file);
	translation = free_stuff(translation);
}
...
void here_we_go(char *file_name, t_flags	*has_flag)
{
    // int file;
    char *buffer;
    ...
    has_flag->visible = 1;
    translate_and_write_to_file(container->tokens, container->labels, container->parameters, has_flag);
    if (container->parameters)
	{
    	free(container->parameters);
		container->parameters = NULL;
	}
	container->tokens = free_stuff(container->tokens);
    if (container)
    	free(container);
    if (buffer)
    	free(buffer);
