...
int get_absolute_distance_to_the_label(t_token *token, char *label_name, t_container *container)
{
...
t_generic_list *tokens, t_container *container)
{
    ...
    absolute_distance = get_absolute_distance_to_the_label(current_token, label_name, container);
    ...
    if (distance_to_the_previous_operation < 0)
        invoke_error("this is broken!", current_token, NULL, NULL); //debug message;
